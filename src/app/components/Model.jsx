...
const Modal = ({ isOpen, onClose, children }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-transparent bg-opacity-50">
      <div className=" bg-amber-200 rounded shadow-lg h-[500px] w-full max-w-3xl p-6 relative">
        <button onClick={onClose} className="absolute top-3 right-3 text-gray-600 hover:text-gray-900">X</button>
        {children}
      </div>
    </div>
  );
...
export default function Navbar() {
  ...
  const fetchPayments = async () => {
    try {
      const token = localStorage.getItem("token");
      const res = await fetch("/api/payments", { method: "GET", headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleNext = () => { if (indexOfLast < filteredPayments.length) setCurrentPage(prev => prev + 1); };

  // Filtering
  const filteredByType = payments.filter(p => viewType === "Loan" ? p.paymentType?.includes("Loan") : !p.paymentType?.includes("Loan"));
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      ...
      <motion.aside initial={{ x: -250, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: 0.6, ease: "easeOut" }} className="w-[280px] bg-white text-gray-800 flex flex-col p-6 shadow-2xl  border-r border-gray-200">
        ...
        <div className="mt-3">
          <button onClick={() => toggleMenu("admin")} className="flex items-center justify-between w-full px-3 py-2 rounded hover:bg-gray-200 transition">
            <span className="flex items-center gap-2 font-bold "><FiSettings size={18} /> Admin Control</span>
            {openMenu === "admin" ? <FiChevronDown /> : <FiChevronRight />}
          </button>
          {openMenu === "admin" && (
            <div className="ml-6 mt-2 flex flex-col gap-2">
              <button onClick={handleAdminForm} className="px-4 py-2 rounded flex items-center gap-2 hover:bg-gray-100"><FiPlus size={16} /> Form</button>
      ...
      {/* Modal */}
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)}>{modalContent}</Modal>
    </div>
  );
}
